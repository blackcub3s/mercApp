...
\begin{document}
	...
	\chapter{Identificación de objetivos}
		...
		Las ventajas de la adopción masiva del ticket digital para el supermercado y el trabajador también son claras: se evita derrochar papel, tener a los cajeros expuestos a químicos que constituyan un riesgo laboral y, finalmente, acortar los tiempos de cola mejorando la experiencia de los clientes y asegurando su regreso futuro.
	...
	\chapter{Diseño del proyecto}  %OBLIGAT
		...
		El tiempo de formación inicial en estas prácticas ha entrañado un curso de Spring Boot excesivamente introductorio, y se estimó que la única forma de ganar conocimientos reales era desarrollar una aplicación de back-end completa y segura con el framework, desde cero. De este modo, el salto futuro a una posición más integrada en el equipo de prácticas y tener oportunidades profesionales al terminar las prácticas serían mayores. Asimismo, esto también ha permitido poder combinar la parte de desarrollo del back-end en Java Spring Boot con el primer mes de jornada de prácticas en Laberit, cumpliendo así un doble objetivo:
			
			\begin{itemize}
				\setlength{\itemsep}{-.2em}
				\item Realizar el curso con garantías reales de aprendizaje.
				\item Realizar la parte de back-end de este proyecto, en paralelo.
				
			\end{itemize}

			
		 permitiendo su consecución en el estrecho plazo que hay en el calendario para hacer tanto las prácticas a jornada completa como este proyecto que, tal y como se ha planteado, ha supuesto bastantes más horas de desarrollo que las que se planeaban por calendario.
		...
		\section{Requisitos Funcionales}
			...
			\textbf{REQUISITO E:} Hacer un sistema front-end y back-end que permitan redirigir a los usuarios rápidamente a un registro de forma inteligente. Nos inspiraremos en el sistema de registro e iniciar sesión de NetFlix.
		...
		\section{Stack tecnológico}
			...
			Se han usado HTML, \textit{vanilla} CSS y \textit{vanilla} JavaScript. Excepciones al uso de los lenguajes puros en el front-end, para la visualización de gráficos se usarán una librería javascript, \href{https://www.chartjs.org/}{chart.js}\cite{chartjs}  que se utilizará para hacer muchos de los gráficos; y para conseguir transiciones usaremos \href{https://animate.style/}{animate.css}\cite{animatecss}
			
			Para el sistema de registro e iniciar sesión se ha hecho una réplica mediante desarrollo inverso de los procesos de registro e inicio sesión de Netflix y se ha adaptado a este caso particular (puede verse en \ref{sec:EnrutamientoDeVistas}). En este paso se ha dedicado muchísimo tiempo y es quizás la parte más importante de este proyecto en cuanto a vínculos directos con los objetivos del CFGS de DAW.
	...
	\chapter{Desarrollo del proyecto} %OBLIGAT
			...
			\section{Desarrollo back-end (Spring Boot)}
				...
				La parte de mercApp programada con Java (Spring Boot) se puede ver en el repositorio de GitHub del proyecto: \href{https://github.com/blackcub3s/mercApp/tree/main/APP%20WEB/__springboot__produccio__/app/src/main}{aquí}. En este link veréis dos rutas a tener en cuenta cuando se corra el proyecto con intelliJ\footnote{Si el lector desea correr el proyecto en intelliJ se recomienda abrir el proyecto en este nivel de altura del árbol de directorios en \textit{\_\_springbootproduccio\_\_/app}.}:
				
				\vspace{-.5em}
				\begin{itemize}
					\setlength{\itemsep}{-.5em}
					\item \textbf{src/main/java} $\rightarrow$ En esta ruta encontramos los \textit{packages} y las clases del proyecto dentro (ver figura \ref{fig:estrucutraAplicacioJAVARESOURCES} recuadro en \textcolor{red}{rojo})
					\item \textbf{resources} $\rightarrow$ Dentro encontramos el archivo \texttt{application.properties}, para definir conexionado con base de datos y que trae por defecto Spring Boot (ver figura \ref{fig:estrucutraAplicacioJAVARESOURCES} recuadro en \textcolor{green}{verde}) y logback-spring.txt que si el lector cambia su extensión de \textit{.txt} a \textit{.xml} va a configurar que los logs o errores del proyecto se guarden
				...
				\begin{figure}[H]
					\centering
					\caption{Estructura del proyecto TO DO IMATGES A LA DRETA}
					\includegraphics[width=.2\textwidth]{img/estrucutraAplicacioJAVARESOURCES.png}
				...
				- Todos los campos de formulario de entrada se han validado no solamente en el front-end sino también en el back-end (hay redundancia de validaciones, por si un usuario maliciosamente intenta hacer llamadas directas a API esquivando las protecciones de front-end): para ello hay archivos\textbf{ TAL TAL TAL} que referenciamos en detalle en \ref{sec:validacioDadesBACK}.
...
\end{lstlisting}
	...
	\begin{figure}[H]
		...
		\caption{Detalle de la landing page \texttt{pas1\_LandingSignUp.html} donde el usuario introducirá inicialmente su correo para registrarse -aunque ese mismo formulario en realidad nos servirá para todo gracias al enrutamiento de vistas-}
	...
	El proceso de enrutamiento de los usuarios desde que llegan a nuestra aplicación web hasta que acceden al \textit{dashboard} se encuentra recogido en el diagrama de la figura \ref{fig:diagramaMercaAppFront}, cuyas nomenclaturas explicamos en los siguientes \textit{bullet points}:
	\vspace{1em}
	\hrule
	...
	\begin{itemize}
		...
		\item Las APIs que consume el front-end van entre [] y con color:
		\begin{itemize}
			\setlength{\itemsep}{.0em}
			\item \textbf{\textcolor{red}{[\texttt{/api/avaluaUsuari}]}}: evalúa si el correo electrónico introducido pertenece a un usuario registrado y qué permisos tiene.
			\item \textbf{\textcolor{brown}{[\texttt{/api/registraUsuari}]}}: registra un nuevo usuario en el sistema y expide su AccessToken con permisos a 0 en las \textit{claims} de su \textit{payload}\footnote{El lector puede consultar la explicación sobre lo que son las claims y el payload de un JWT en el apartado \ref{sec:queComponeJWTbackend}.}.
	...
	\subsubsection{Protegiendo las vistas: expiración del token}
	
	Existen dos páginas de nuestra web que, cuando expire su token de acceso, no deberán ser visualizables\footnote{Como se desprende a partir del diagrama de flujo del enrutamiento que vimos en la figura \ref{fig:diagramaMercaAppFront}, son las páginas a las que redirigimos los usuarios justo después de generar tokens de acceso.}:
	...
	La primera requiere tener un token con permisos a 0; la segunda un token con permisos superior o igual a 1. En otras palabras: requieren tener algun tipo de autenticación de usuario, como veremos en el siguiente apartado; pero está claro que para visualizarlas la condición \textit{sine qua non} comuna es que dentro de \textit{localStorage.getItem(``AccessToken''}) deberán tener un token que no esté expirado y que sea descifrable (ojo, que no validable con el secret, que eso está en nuestro servidor y no se muestra jamás en el cliente).
	...
	<script src="/js/jwt/restringeixVistes_EXPIRACIO.js"></script>
	\end{lstlisting}
	
	En el primero tenemos la función para extraer el payload de un token. Y en el segundo está la lógica que acabamos de explicar en los párrafos anteriores (ver figura \ref{fig:restringeixVistesEXPIRACIO}):
	...
	\begin{figure}[H]
		\centering
		\caption{Script para regresar automáticamente a la landing page cuando el token de acceso expira en páginas protegidas.}
		\includegraphics[width=1\linewidth]{img/restringeixVistesEXPIRACIO}

		\label{fig:restringeixVistesEXPIRACIO}
	\end{figure}
	\FloatBarrier
...
\textit{NOTA: En este trabajo no implementaremos cookies ya que implica configuración extra tanto en el cliente como en el servidor. Vamos a guardar el token en el cliente en el localStorage (que es, de hecho, una práctica habitual en aplicaciones que no requieren un alto grado de seguridad). También hay que mencionar sobre que existe un debate para ver si en ese logIn el token de acceso recién generado en el servidor se debe mandar al cliente en el body de la respuesta de la solicitud POST \textit{o bien} en la header ``Authorization''. Sin embargo, es práctica común mandarlo en el body. Nótese, que para el paso inverso (cliente a servidor) sí debe mandarse en el Heather ``Authorization''. }
