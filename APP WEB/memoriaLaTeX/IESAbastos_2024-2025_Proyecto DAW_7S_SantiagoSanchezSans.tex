...
\end{lstlisting}
	...
	NOTA: Podéis ver la función completa del controlador en el anexo \ref{sec:detallSeguretatControlador}
			...
			La parte donde definimos la validación de datos de los endpoints de entrada de los datos de usuario está en utils\footnote{podéis ver la imagen completa de la estructura del proyecto SpringBoot si queréis, de nuevo, en \ref{fig:estrucutraAplicacioJAVARESOURCES}} pero se produce parte de validación a través de \textit{Usuaris} $\rightarrow$ \textit{dto}. El esquema para entender como se interelacionan estas clases está en la figura \ref{fig:validacioBackArxius}.
			...
			Por ejemplo, para validar los datos de entrada a través del endpoint que recibe llamada del front-end para el incio de sesión, es decir, el endpoint ``\href{https://github.com/blackcub3s/mercApp/blob/4ddc34194763af7a246ffabb14146ad9b4b2c5db/APP%20WEB/__springboot__produccio__/app/src/main/java/miApp/app/Usuaris/controlador/UsuariControlador.java#104}{api/login}'' del ControladorUsuari.java de Spring Boot, podréis ver que el tipo de datos que espera este endpoint en el \textit{@RequestBody }(anotación que pide cargar en el parámetro ``dto'' un tipo de datos que se introducirá ahí desde el body de la solicitud POST) no es un tipo de datos primitivo, ni una clase predefinida de java: sino una clase definida por nosotros. A esa clase la hemos llamado LoginDTO. Los data transfer objects o DTOs son simplemente clases Java, y mediante sus atributos definen los nombres exactos que tienen que tener las claves entrantes del body de las solicitudes POST y aplican restricciones a los valores que éstas llevan asociadas\footnote{Acordémonos que las entradas de datos por por el cuerpo de una solicitud POST se ponen formato JSON por convenio} según unas expresiones que definimos dentro de cada clase de anotación, si y solo si, añadimos la anotación \textbf{@Valid}: con esta anotación activamos las clases de anotación que ponemos encima de los atributos de los DTOs, que son las que hacen la validación de datos. En la imagen \ref{fig:validacioBackArxiusDetallLogin} siguiente se ve muy claro qué es lo que realmente está pasando:
			...
			\begin{figure}[H]
				\centering
				\caption{\textbf{TO DO}explicar}
				\includegraphics[width=1\linewidth]{img/validacioBackArxiusDetallLogin}
				\label{fig:validacioBackArxiusDetallLogin}
			\end{figure}
			\FloatBarrier
			...
			Podría parecer complejo hacer lo que hemos hecho en la figura \ref{fig:validacioBackArxiusDetallLogin}, pero una vez guardarmos campos en la base de datos a través de múltiples endpoints que requieren modificaciones parciales de esos campos hacerlo así tiene utilidad y es una buena práctica en Java.
