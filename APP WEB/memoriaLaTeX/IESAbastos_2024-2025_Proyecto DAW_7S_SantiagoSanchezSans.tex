...
\end{lstlisting}
	...
	NOTA: Podéis ver la función completa del controlador en el anexo \ref{sec:detallSeguretatControlador}
			...
			Por ejemplo, para validar los datos de entrada a través del endpoint que recibe llamada del front-end para el incio de sesión, es decir, el endpoint ``\href{https://github.com/blackcub3s/mercApp/blob/4ddc34194763af7a246ffabb14146ad9b4b2c5db/APP%20WEB/__springboot__produccio__/app/src/main/java/miApp/app/Usuaris/controlador/UsuariControlador.java#104}{api/login}'' del ControladorUsuari.java de Spring Boot) podréis ver que el tipo de datos que espera este endpoint en el @RequestBody (anotación que pide cargar en el parámetro ``dto'' un tipo de datos que se introducirá ahí desde el body de la solicitud POST) no es un tipo de datos primitivo, ni una clase predefinida de java: sino una clase definida por nosotros. A esa clase la hemos llamado LoginDTO. Los data transfer objects o DTOs definen mediante atributos de clase Java los nombres que tienen que tener las claves entrantes del body de las solicitudes POST y aplican restricciones a los valores que estas continen según unas expresiones que definimos dentro de cada clase de anotación. En la imagen \ref{fig:validacioBackArxiusDetallLogin} siguiente se ve muy claro qué es lo que realmente está pasando:
