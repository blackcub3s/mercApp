...
\textit{NOTA: En este trabajo no implementaremos cookies ya que implica configuración extra tanto en el cliente como en el servidor. Vamos a guardar el token en el cliente en el localStorage (que es, de hecho, una práctica habitual en aplicaciones que no requieren un alto grado de seguridad). También hay que mencionar sobre que existe un debate para ver si en ese logIn el token de acceso recién generado en el servidor se debe mandar al cliente en el body de la respuesta de la solicitud POST \textit{o bien} en la header ``Authorization''. Sin embargo, es práctica común mandarlo en el body. Nótese, que para el paso inverso (cliente a servidor) sí debe mandarse en el Heather ``Authorization'' con el preámbulo ``Bearer '' seguido del token. }
	...
	Para hacer esta parte es importante mencionar que tenemos que configurar la consola de google cloud. En última instancia tenemos que obtener un (\textbf{Client ID}) que identifique nuestra web en relación a google cloud y permita a los usuarios de nuestro sitio web (en el \href{https://github.com/blackcub3s/mercApp/blob/main/APP%20WEB/__frontend__produccio__/app/pas4_concedirAccesGmail.html}{\texttt{pas4\_concedirAccesGmail.html}}) descargar sus tickets digitales de su correo gmail personal hacia su ordenador (haciendo uso de javascript). 
	
	La obtención del Client ID y las configuraciones pertinentes en google cloud las mostramos en el anexo \ref{sec:googleCloud}. Una vez obtenido el Client ID, haremos uso de un script auxiliar que nos ha proporcionado chatGPT (\href{https://github.com/blackcub3s/mercApp/blob/main/llavorsGPT/extraccionCorreosPdfGoogleCloud.html}{\texttt{semilla})}. Ese script es un esqueleto que ha sido modificado de modo que se ajuste a nuestras necesidades hasta llegar al archivo javascript definitivo (\textbf{link a javascript}).
...
\end{lstlisting}
	...
	\section{Google cloud: descargar tikets digitales}
	\label{sec:googleCloud}
		Los pasos a seguir son como prosiguen:
	
	
	\noindent \textbf{PASO 1: Habilitar la Gmail API}
	\vspace{.2em}
	\hrule
	\vspace{.5em}
	...
	\noindent \textbf{PASO 2: Configurar la pantalla de consentimiento OAuth}
	\vspace{.1em}
	\hrule
	\vspace{.5em}
	...
	\noindent \textbf{PASO 3: Crear credenciales OAuth 2.0 para Web}
	\vspace{.1em}
	\hrule
	\vspace{.5em}
