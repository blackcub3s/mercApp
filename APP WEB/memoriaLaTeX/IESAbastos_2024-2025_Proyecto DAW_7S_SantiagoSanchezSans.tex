...
\textit{NOTA: En este trabajo no implementaremos cookies ya que implica configuración extra tanto en el cliente como en el servidor. Vamos a guardar el token en el cliente en el localStorage (que es, de hecho, una práctica habitual en aplicaciones que no requieren un alto grado de seguridad). También hay que mencionar sobre que existe un debate para ver si en ese logIn el token de acceso recién generado en el servidor se debe mandar al cliente en el body de la respuesta de la solicitud POST \textit{o bien} en la header ``Authorization''. Sin embargo, es práctica común mandarlo en el body. Nótese, que para el paso inverso (cliente a servidor) sí debe mandarse en el Heather ``Authorization'' con el preámbulo ``Bearer '' seguido del token. }
	...
	Para ello Se utilizará el remitente de la dirección  de correo electrónico desde la que Mercadona nos manda los tickets digitales para poder filtrar los correos y descargar sus  pdfs. La dirección es: \textbf{ticket\_digital@mail.mercadona.com} y las línea de código donde conseguimos esta descarga las marcamos en este  \href{https://github.com/blackcub3s/mercAppAuxPas4/blob/03deec7187c1194e20d324830b7c8ea8f4d3671b/js/scriptExtraccio.js#L89-L92}{snippet} de github (nótese que en este script solamente descargamos 1 solo correo). Esto es porque solamente descargando correos de 5 en cinco de golpe, ya consumimos el free tier que nos ofrece google: 300 dólares. Esto es porque para testear en ese commit nos servia, por aquel entonces.
	
	A medida que hemos ido desarrollando hemos llegado a lo siguiente: Si descargamos 500 pdfs de golpe, Google puede ralentizar las descargas o impedir que sigamos descargando. Esto pasa porque existen unas cuotas máximas que podemos usar para descargar. En la siguiente página de  developers.google \cite{gmail_api_quota} (\href{https://developers.google.com/workspace/gmail/api/reference/quota?hl=es-419}{link}) podemos ver que cada una de las funciones que usamos en el script par extraer los correos electrónicos consumen ciertas unidades de quota. Nosotros usamos \textbf{messages.attachments.get}, \textbf{users.messages.list} y \textbf{messages.attachments.get}, que tienen un uso de quotas por cada llamada cada uno de 5 unidades. Es decir, cada ticket descargado para un usuario nos va a consumir un total de 15 unidades. Si ese mismo usuario se descarga 500 tickets digitales adjuntos de los correos en un segundo -la api va rápida- supondrán 7500 unidades de cuota gastadas de golpe. Si el usuario decide volver a autenticarse y volver a descargar va a gastar las 15 000 unidades y fácilmente podrá excederse de esta cuota y nos daría \textit{userRateLimitExceeded}. Esto es un aspecto que en una versión más definitiva deberíamos pulir (se puede añadir batch processing, para descargar por lotes si queremos aumentar el límite de tickets digitales descargables) pero para la versión que nos ocupa no vamos a hacerlo. Asimismo, el límite de frecuencia global por proyecto son 1 200 000 unidades de cuota por minuto, lo cual nos daría la posibilidad de tener hasta 160 usuarios concurrentes en un minuto suponiendo que cada uno de ellos hasga como máximo una descarga de 500 tikets \footnote{1200000 (uc/min)/7500 (uc/usuario) = 160 usuarios/min} sin dar \textit{rateLimitExceeded}. En la figura \ref{fig:quotesGoogleDevelopers} siguiente podéis ver los límites de la página de desarrolladores de google que enlazamos antes y en la figura \ref{fig:quotesGoogleDevelopersUSADES}
