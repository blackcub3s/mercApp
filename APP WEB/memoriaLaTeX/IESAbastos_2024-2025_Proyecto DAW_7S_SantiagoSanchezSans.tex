...
\textit{NOTA: En este trabajo no implementaremos cookies ya que implica configuración extra tanto en el cliente como en el servidor. Vamos a guardar el token en el cliente en el localStorage (que es, de hecho, una práctica habitual en aplicaciones que no requieren un alto grado de seguridad). También hay que mencionar sobre que existe un debate para ver si en ese logIn el token de acceso recién generado en el servidor se debe mandar al cliente en el body de la respuesta de la solicitud POST \textit{o bien} en la header ``Authorization''. Sin embargo, es práctica común mandarlo en el body. Nótese, que para el paso inverso (cliente a servidor) sí debe mandarse en el Heather ``Authorization'' con el preámbulo ``Bearer '' seguido del token. }
	...
	Al pinchar en el círuclo verde de la imagen \ref{fig:googleCloudD} obtenemos una ventana donde podemos configurar el \textit{ClientID}\footnote{Podemos llegar a la \textbf{ventana B} de la figura \ref{fig:googleCloudE} del mismo modo si optamos por seguir una ruta por menús. Ponemos en el buscador ``Apis y servicios'' y acto seguido nos embarcamos en el camino: \textit{Apis y servicios} $\rightarrow$ \textit{credenciales} $\rightarrow$ \textit{+ crear credenciales} $\rightarrow$ \textit{ID de cliente Oauth}.}. El clientID identifica la aplicación que corre en el cloud que estamos creando. Sin él un usuario no podría llegar a hacer una llamada a nuestro cloud autenticándose con su cuenta de google. Al configurarlo (como podemos ver en la imagen \ref{fig:googleCloudE}) nosotros tenemos que decirle a Google de qué dominios y puertos debe permitir llamadas.
	...
	Después de todo esto la vista que tendremos si entramos en el buscador, en \textbf{credenciales} será esta:
	...
	\noindent \textbf{PASO 5: Validar la aplicación descargaTicketsGmail para que cualquier usuario externo pueda usarla.}
	\vspace{.1em}
	\hrule
	\vspace{.5em}
		...
		Y el último caso es validar. Que google tiene que dar el vistobueno. Sin embargo asumo que esto no va a pasar, al menos no en plazo, con lo cual, directamente añadiremos mi correo para descargar los tikets digitales y vamos por ahora a borrar los permisos restringidos para que google deje usar la aplicación en la demo.
		...
		\begin{figure}[H]
			...
			\caption{Eliminamos el permiso restringido para que la demo funcione (izquierda); dejamos fe de los pasos seguidos hasta ahora para en un futuro tener una versión de producción. Acto segido añadimos mi correo electrónico donde figuran los tickets digitales de mis compras de mercadona, con el que haremos la extracció de tickets (Derecha).}
