...
\end{lstlisting}
		...
		Cuando el usuario consigue poner la contraseña correcta en la página de registro del front-end \texttt{pas3C\_crearContrasenya.html} se manda de vuelta del back-end (desde el controlador de endpoint \textit{api/registraUsuari}) hasta el front-end en el localStorage, \textbf{el token de acceso}. Pasa lo mismo cuando iniciamos sesión, en \texttt{pas2C\_login.html}.
		
		
		Así las cosas, podemos testear el endpoint de \textit{api/registraUsuari} ehaciendo una llamada POST desde postman asegurándose que ``nuevoUsuario@gmail.com'' no existe en la tabla usuaris para luego testearlo con el front-end, tal que así:
		...
		\begin{figure}[H]
			...
			\caption{Creación de un nuevo usuario llamando con una solicitud post al endpoint ``api/registraUsuari'' mandando los datos de ``correuElectronic'' y ``contrasenya'' que leerá y validará el DTO. En naranja es el body de la solicitud y en verde el body de la response}
		...
		Análogamente, para testear el endpoint de iniciar sesión sin utilizar el front-end podemos hacerlo así:
		...
		\begin{figure}[H]
			...
			\caption{Iniciando sesión con un usuario mediante ``api/login'' mandando los datos de ``correuElectronic'' y ``contrasenya'' que leerá y validará el DTO del back-end. En naranja es el body de la solicitud y en verde el body de la response. El correo ya debe existir en la base de datos!}
...
\end{lstlisting}
	...
	El proceso de registro que utilizamos aquí es, justamente, una derivación del mismo: pero en este caso no se utilizan redirecciones a cartas de pago sino que redirigimos al usuario a una de dos páginas posibles según su caso particular con los permisos. Una, es una página para darnos acceso al gmail donde Mercadona envía al usuario los tickets digitales para así poder nosotros descargarlos y procesarlos: \texttt{pas4\_ConcedirAccesGmail.html}; la otra es \texttt{dashboard.html}, donde se visualizan el resultado de la minería y extracción de datos de esos tikets. El proceso se encuentra en el diagrama de la figura \ref{fig:diagramaMercaAppFront}, cuyo contenido explicamos a continuación:
	...
	\begin{itemize}
		...
		\item 4. Expedición de tokens de acceso (JWT) desde el endpoint (\textbf{\textcolor{blue!40!white}{\underline{rectangulo de fondo azul}}}) enviados al front-end \footnote{Consultad la estructura de los mismos en la figura \ref{fig:jwtioMostraPayload}.}.
