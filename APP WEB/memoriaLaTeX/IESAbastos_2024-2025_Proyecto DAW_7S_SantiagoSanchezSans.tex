...
\textit{NOTA: En este trabajo no implementaremos cookies ya que implica configuración extra tanto en el cliente como en el servidor. Vamos a guardar el token en el cliente en el localStorage (que es, de hecho, una práctica habitual en aplicaciones que no requieren un alto grado de seguridad). También hay que mencionar sobre que existe un debate para ver si en ese logIn el token de acceso recién generado en el servidor se debe mandar al cliente en el body de la respuesta de la solicitud POST \textit{o bien} en la header ``Authorization''. Sin embargo, es práctica común mandarlo en el body. Nótese, que para el paso inverso (cliente a servidor) sí debe mandarse en el Heather ``Authorization'' con el preámbulo ``Bearer '' seguido del token. }
	...
	El icono para cerrar sesión es el mismo que en el dashboard y se han eliminado todos los links de la navBar. Luego se ha definido un grid con dos columnas, alternando imágenes y texto, tal que así:
	
	\FloatBarrier
	\begin{figure}[H]
		\centering
		\caption{Vista del grid de dos columnas del pas4 (versión definitiva puede variar ligeramente en su padding)}
	...
	Un aspecto diferencial con el \textbf{dashboard} es que el diseño responsive de la parte clave ha implicado generar una media query en la que le pedíamos en los selectores descendientes \textbf{TO DO} que invirtieran el orden de las imágenes: nótese que en el HTML la imagen de descarga de tickets y la de ``relájese'' aparecen en orden inverso porque queremos alternar los iconos y el texto en la vista de escritorio en un grid de dos columnas. Sin embargo, este diseño de escritorio hace que al ponerlo en vertical en un grid de una sola columna tanto el icono de ``descarga tickets'' como el de ``relájese'' aparezcan en orden inverso. La solución está en ahcer que la primera fila del grid en esos casos, que es el texto, ocupe la segunda posicion mediante la propiedad ``grid-row: 2'' como podemos ver en la imagen \ref{fig:pas4grid1col}
	
	\FloatBarrier
	\setlength{\belowcaptionskip}{3pt}
	\begin{figure}[H]
		\centering
		\caption{Media query para diseño responsive del grid de dos columnas de la versión desktop, convertido en un grid de uan columna en la versión definitiva: uso de ``grid-row''.}
		\includegraphics[width=1\linewidth]{img/pas4grid1col}
		\label{fig:pas4grid1col}
	\end{figure}
	\FloatBarrier
	
	Finalmente lo que sí es una diferencia en relación al dashboard es que se ha automatizado la propiedad dropshadow utilizando JavaScript en lugar de haciéndolo con CSS. La idea era crear una sombra que fuese reactiva al ratón, automatizada para que genere una sombra de dirección opuesta a la desviación que hay entre el centro del icono y el punto donde está el ratón en caso que se emplace por encima de este icono.
	
	\href{https://github.com/blackcub3s/mercApp/blob/main/APP%20WEB/__frontend__produccio__/app/js/pas4/dropShadowReactiu.js}{Este script} no fue programado a mano, sino que se generó simplemente con un prompt, como podemos ver en la figura \ref{fig:promptScriptDropShadow}. Luego se ajustaron los valores de blur del dropshadow, la división del vector opuesto para evitar generar una sombra demasiado grande y se añadió un event listener de tipo ``DOMContentLoaded'' para que cargue después del dom. El propio script se consigue con los eventos 
	...
	La extracción de tickets \underline{no se puede hacer en local con javascript} porque las políticas de seguridad de de google le dicen al navegador que impida las llamadas a su API (se activa algo llamado la ``Content-Security-Policy''): esto pasa porque, entendemos, al no tener HTTPS el origen no es seguro. Debe hacerse, por lo tanto, desde un script en remoto, en un dominio autorizado en internet, con protocolo HTTPS y autorizado manualmente desde la configuración de la consola de Google (Que ya hemos hecho). Para ello se ha creado este repositorio para hacer descargas de tickets (\href{https://github.com/blackcub3s/mercAppAuxPas4/}{repo mercAppAuxPas4}).
	
	De este repo, el único archivo que usaremos para que podamos alimentar nuestro archivo \texttt{pas4\_concedirAccesGmail} corriendo en localhost será este: \href{https://github.com/blackcub3s/mercAppAuxPas4/blob/main/js/scriptExtraccio.js}{scriptExtraccio.js}. Para poder correr el script usaremos su ruta en GitHub Pages (no la ruta del repositorio GitHub, sino la ruta que tiene el archivo cuando se carga en la página de GitHub Pages que hace el deploy de este mismo repositorio). Esto lo haremos haciendo uso de la URL del repositorio, pero sin el grupo ``blob/main'' en ella. Luego, lo cargaremos en \texttt{pas4\_concedirAccesGmail.html} como si fuera servido de una CDN externa\footnote{Solo que la CDN la hemos creado expresamente para la ocasión.}:
