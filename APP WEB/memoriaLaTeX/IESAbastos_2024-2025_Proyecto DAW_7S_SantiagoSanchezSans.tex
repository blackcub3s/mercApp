...
\begin{document}
	...
	\chapter{Desarrollo del proyecto} %OBLIGAT
			...
			Se establecen tres niveles de permisos en la aplicación que tienen un impacto en qué puede visualizar el usuario logueado y qué no; y cómo se permite que el usuario navegue a medida que va moviéndose en el proceso de registro. Estos permisos los podemos ver en la parte superior derecha de la figura \ref{fig:diagramaMercaAppFront}.
						
			\begin{table}[H]
				\centering
				\caption{Significado del valor de la variable \texttt{permisos} en la tabla \texttt{usuaris} (ver figura~\ref{fig:diagramaMercaAppFront}) para ver como esto redirige al usuario en el enrutamiento de la aplicación para el proceso de registro}
				\begin{tabular}{|c|l|}
					\hline
