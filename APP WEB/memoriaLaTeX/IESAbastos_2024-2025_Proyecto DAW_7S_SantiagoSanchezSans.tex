...
docker create -p 8000:8000 --name contFastApi back-end-fastapi
	...
	\begin{figure}[H]
		\centering
		\caption{definición de un endpoint de fastApi que correrá dentro del contenedor docker contFastApi A), configuración del comando para correr fastAPI dentro de los contenedores que se instancien a partir de la imagen del Dockerfile, con la dirección de red 0.0.0.0 que permite salir fuera de los contenedores que se creen con la misma (B) rectángulo rojo) y configuración de que fastAPI correrá en el puerto 8000 de \textbf{dentro} del contenedor (B) rectángulo verde). Finalmente podemos ver el comando que crea el contenedor didiendo que escuche en el 8000 de la red interna del contenedor (C) rectángulo verde y que esto lo saque al puerto 8000 (C naranja) siendo el resultado observable en nuestro equipo anfitrión del contenedor en el localhost en ese mismo puerto 8000 (D, naranja).}
