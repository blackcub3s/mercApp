		...
		\FloatBarrier
			...
			\subsection{Camino recorrido durante el registro de usuario}
				...
				\begin{figure}[H]
					...
					\caption{Simplificación de los caminos activados en el diagrama de sistemas durante el registro de un usuario, hasta que este tiene acceso al dashboard de visualización con éxito en todas las partes del proceso: a) Usuario pone correo y contraseña. b) guardamos correo y hash contraseña. c) Mandamos token de acceso con permisos = 0 y pedimos al usuario que nos conceda acceso a sus tickets digitales. d) usuario se autentica con \textbf{google API client} en la Gmail API con OAuth2 e) tickets regresan al navegador y se descargan al ordenador del usuario. f) fastAPI recibe los tickets del ordenador del usuario y los parsea a formato JSON estructurado. g) Los tickets estructurados se guardan en MongoDB h) fastAPI pasa el token de acceso con permisos a 0 hacia Spring Boot y le pide uno con permisos a 1. i) Spring Boot guarda permisos a 1 a la BBDD mySQL j) Spring Boot regresa a fastAPI el token de acceso recién expedido con permisos a 1. k) FastAPI manda el token de acceso con permisos = 1 de vuelta al frontend (RESPONSE) para que JavaScript muestre automáticamente el dashboard con el análisis de los tikets digitales.}
