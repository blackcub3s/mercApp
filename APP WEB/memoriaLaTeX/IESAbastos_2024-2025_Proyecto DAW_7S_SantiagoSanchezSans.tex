...
<script src="https://blackcub3s.github.io/
	...
	\noindent \textbf{PARTE2: Subir tickets digitales del ordenador del usuario al sistema de archivos del proyecto de fastAPI (llamada JavaScript $\rightarrow$ \textit{api/subir-tickets-pdf})}
	\hrule
	\vspace{.5em}
	...
	Al clicar en ese icono activaremos un evento change que abrirá un cuadro de diálogo para subir los pdfs, y cuando le demos a abrir automáticamente hará una solicitud fetch al endpoint \textit{/api/subir-tickets-pdf} del controlador de fastAPI: subiendo los pdfs al sistema de archivos -aquí todavía no queremos guardado en base de datos- (el codigo javascript que lo consigue conforma íntegramente todo el contenido del archivo \href{https://github.com/blackcub3s/mercApp/blob/main/APP%20WEB/__frontend__produccio__/app/js/pas4/pujaPdfs.js}{pujaPdfs.js}, que es donde está la solicitud POST con la función fetch correspondiente).
	
	Nótese que para subir PDFs necesitamos un formulario: normalmente usaríamos el botón que se genera con la etiqueta HTML \textit{input}. Pero en este caso no hay ni rastro de él. Esto es porque es la propia imagen la que hace la función de botón \textit{input}: es mediante click a ella que se consigue llamar a la etiqueta input, gracias a que está anidada en la etiqueta \textit{label} y que usamos el atributo for que la enlaza mediante id a la etiqueta \textit{input} (véase imagen \ref{fig:pujarTicketsAuxiliarLabelHTML}).
	...
	Una vez terminada la parte 3 el navegador recibe token de acceso con permisos 1 (y se redirige automáticamente al dashboard, como ya vimos en los scripts de protección de vistas públicas)
