...
\begin{document}
	...
	\node (asterisc2Explicacio)[aspecteExplicacioAsterisc, right of = asterisc2Arribada, yshift=-2.2cm, xshift=1.4cm] {
		\begin{tabular}{l}
			Si permisos == 0:\\
				\hspace{1.9em} No acceso a tickets digitales\\\hspace{2em}\textit{[rol $\rightarrow \emptyset$ ]} $:$ \textbf{  NO Acc. APIs}\\
			Si permisos $==$ 1:\\
				\hspace{1.9em} Hay acc. a sus tickets digitales\\\hspace{2em}\textit{[rol $\rightarrow$ USER]} $:$ \textbf{ Acc. APIs idUsuario}\\
			Si permisos $==$ 2:\\
