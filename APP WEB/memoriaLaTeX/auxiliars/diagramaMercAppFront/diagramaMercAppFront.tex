...
\begin{document}
	...
	\node (concedido) [decisionFrontend, below of=pas4, yshift=-2cm] {concedido};
	...
	\node (jwtLOGIN2) [nodeAccessToken, below of=dec3, yshift=-8cm, text width=3cm] {\textbf{AccessToken} \\$permisos = 0$};
	...
	\node (invisible10) [nodeNoDimensio, above of=jwtLOGIN2, yshift=0cm]{};
	...
	\node (dummy9) [below of=dec3, xshift=0cm, yshift=0cm] {\textbf{\textcolor{brown}{[/api/registraUsuari]}}};
	...
	\node (asterisc2Explicacio)[aspecteExplicacioAsterisc, right of = asterisc2Arribada, yshift=-1.9cm, xshift=1.15cm] {
		\begin{tabular}{l}
			Si permisos == 0:\\
				\hspace{1.9em} No hay acceso a tickets digitales.\\
			Si permisos $==$ 1:\\
				\hspace{1.9em} Hay acceso a tickets digitales\\\hspace{2.5em}\textit{[rol $\rightarrow$ USER]}\\
			Si permisos $==$ 2:\\
				\hspace{1.9em} Usuari tiene acceso a PDFs correo\\\hspace{2.5em}\textit{[rol $\rightarrow$ ADMIN]}\\
	...
	\draw [linia] (dec3) -- (jwtLOGIN2) node[midway, yshift=1cm, xshift=-.3cm] {S\'{i}};
	...
	\draw [linia] (dec5) -- (invisible10) node[midway, yshift=.3cm, xshift=3.5cm] {No};
