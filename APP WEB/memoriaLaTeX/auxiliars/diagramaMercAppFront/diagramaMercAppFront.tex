...
\tikzstyle{nodeAccessToken} = [ellipse, draw=black, fill=red!50, minimum height=1cm, minimum width=2cm, text centered]
...
\begin{document}
	...
	\node (pas4) [trapeziSortida, below of=pas3, yshift=-10.5cm] {pas4\_concedirAccesGmail};
	...
	\node (jwtLOGIN) [nodeAccessToken, below of=dec4, yshift=-1cm, text width=1.3cm] {JWT (Access Token)};
	\node (dec5) [decisionBackend, below of=dec4, yshift=-4cm, text width=1.5cm] {¿Hay acceso a recursos?};
	...
	\node (invisible5) [nodeNoDimensio, below of=pas2b,yshift=-7cm]{};
	...
	\draw [linia] (dec4) -- (jwtLOGIN) node[midway, yshift=0cm, xshift=.3cm] {S\'{i}};
	\draw [arrow] (jwtLOGIN) -- (dec5);
	\draw [arrow] (dec5) -- (dashboard) node[midway, yshift=1.8cm, xshift=.3cm] {S\'{i}};
