...
\begin{document}
	...
	\chapter{Desarrollo del proyecto} %OBLIGAT
			...
			\section{desarrollo back-end}
				...
				\noindent El JWT se compone de tres partes. \textbf{los headers}, \textbf{el payload} y \textbf{la signatura}. En la página \href{https://jwt.io/}{https://jwt.io/}, como veremos después, se pueden validar tokens \cite{jwtio} y ver su estructura interna.
	...
	<dependency>
		...
		En el proyecto se ha creado una clase dentro de \texttt{src/main/java/miApp.app/} \texttt{seguretat} denominada \textbf{JwtUtil}. En esta clase hemos creado tres métodos que trabajan con funciones de la librería \texttt{io.jsonwebtoken} concretamente con el módulo \textbf{Jwts}. La primera de estas funciones
		nos genera el \texttt{access token} a partir de la \textit{secret key} o contraseña secreta del servidor que tenemos en el atributo de la clase:
...
public static String generaAccesToken(String correu, 
                                      int tExpM, 
                                      int idUsuari, 
                                      byte permisos) {
	...
	.compact();
		...
		La segunda función crea un refresh token (no la usaremos en este proyecto pero la hemos hecho para implementarla en un futuro) y la tercera permite obtener el payload (las Claims) y trabajar con la informacion que hay dentro. 
		
		La tercera función \texttt{getClaims()} es la que utilizaremos en el Service de nuestra aplicación para poder autenticar usuarios. La clase puede ser consultada en el anexo \ref{sec:anexoCreacionYverificacionJWT} o en el GitHub del proyecto (\href{https://github.com/blackcub3s/mercApp/blob/main/APP%20WEB/__springboot__produccio__/app/src/main/java/miApp/app/seguretat/JwtUtil.java}{link}). Se recomienda encarecidamente al lector optar por esta última opción\footnote{En esta memoria, al poner la clase JwtUtil.java se omitió la función main donde se testeaban las dos funciones de creación de tokens con control de excepciones, por falta de espacio en el DIN A4.}.
...
\pagebreak
		...
		\pagebreak
		\section{Clase para crear Access tokens, refresh tokens y verificarlos}
...
public class JwtUtil {
	...
	// FINALITAT: Refrescar el token d'acces que genera generaAccesToken().
